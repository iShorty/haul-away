...
public partial class MultiUseStation
{
    ...
    [SerializeField]
    Animator _grapplerAnimator = default;
    ...
    {
#if UNITY_EDITOR
        Debug.DrawRay(_standModel.position, worldSpaceDirNormalized, Color.red, Time.deltaTime);
#endif
        ...
        _standModel.transform.rotation = Quaternion.Slerp(_standModel.transform.rotation, lookRot, Time.deltaTime * Info.RotationSpeed);
        ...
        rotationVector = trueForwardAxis.InverseTransformDirection(worldSpaceDirNormalized);
        rotationVector.x = 0;
        rotationVector = trueForwardAxis.TransformDirection(rotationVector);

        //This always returns positive angle
        float zAngle = Vector3.Angle(trueForwardAxis.forward, rotationVector);
...
#if UNITY_EDITOR
        Debug.DrawRay(trueForwardAxis.position, rotationVector, Color.magenta, Time.deltaTime);
#endif

        rotationVector.y = 0;
        rotationVector.x = zAngle;
