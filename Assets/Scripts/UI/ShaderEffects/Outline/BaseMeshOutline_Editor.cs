...
public partial class BaseMeshOutline
{
    ...
    const int CHANNELSTORED_IN = 1;

    #endregion

    bool _prevUseCalNormals = false;

    [SerializeField]
    ///<Summary> The maximum distance apart two vertices must be to be merged</Summary>
    float _cospatialVertexDistance = 0.01f;

    private void OnValidate()
    {
        if (_prevUseCalNormals != _info.PrecalculateNormals)
        {
            _prevUseCalNormals = _info.PrecalculateNormals;
    ...
    public void CalculateNormal()
    {
        ...
        mesh.SetUVs(CHANNELSTORED_IN, outlineNormals);
    }
    ...
    private bool SearchForPreviouslyRegisteredCV(Vector3 position, List<CospatialVertex> registry, out int index)
    {
        ...
        for (int i = 0; i < registry.Count; i++)
        {
            ...
            if (Vector3.Distance(registry[i].position, position) <= _cospatialVertexDistance)
            {
                index = i;
                return true;
            }
